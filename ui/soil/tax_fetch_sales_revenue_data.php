...
function fetch_sales_data($pdo,$id)
{
	...
	$select = $pdo->prepare("SELECT * FROM tb_tax_sales  ");
	$select->execute();
	while ($row = $select->fetch(PDO::FETCH_OBJ)) 
	{
		...
		$subdata[]= '<center>
						<font style="color:rgb(6, 61, 119, 1);" size="2" >'.number_format($row->gross_amount,2).'</font>
					</center>';

		$subdata[]= '<center>
						<font style="color:rgb(142, 8, 11, 1);" size="2" >'.number_format($row->vat,2).'</font>
					</center>';

		$subdata[]= '<center>
						<font style="color:rgb(55, 142, 55, 1);" size="2" >'.number_format($row->net_amount,2).'</font>
					</center>';

		$subdata[]= '<center>
						<font style="color:rgb(177, 149, 59, 1);" size="2" >'.number_format($row->withholding_total_cwt,2).'</font>
					</center>';

		$subdata[]= '<center>
						<font style="color:rgb(177, 149, 59, 1);" size="2" >'.number_format($row->withholding_total_vwt,2).'</font>
					</center>';
		...
		<center>
			<div class="btn-group">
				...
				<a href="tax_edit_sales_revenue3.php?id='.$row->id.'" class="btn btn-success btn-sm"><span class="fa fa-edit"  data-toggle="tooltip" title="Edit" ></span></a>

				<button data-id='.$row->id.'  class="btn btn-danger btn-sm" id="btn_delete_sales_revenue"><span class="fa fa-trash" style="color:#ffffff" data-toggle="tooltip" title="Delete" ></span></button>
