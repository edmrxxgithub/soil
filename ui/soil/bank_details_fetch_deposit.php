...
// session_start();
	...
	$select = $pdo->prepare("SELECT * FROM tb_deposit WHERE bank_id = '$bankid' ");
	$select->execute();
	while ($row = $select->fetch(PDO::FETCH_OBJ)) 
	{
		...
		$subdata[]= '<center><font class="text-black" size="2">'.$row1->name.' - '.$row1->account_num.'</font></center>';
		...
		<center>
			<div class="btn-group">
				...
				<a href="bank_details_edit_deposit.php?id='.$row->id.'&bankid='.$bankid.'" class="btn btn-primary btn-sm"><span class="fa fa-edit"  data-toggle="tooltip" title="Edit check" ></span></a>
