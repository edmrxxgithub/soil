...
function fetch_total_payment($pdo,$id)
{
	...
	$select = $pdo->prepare("SELECT * FROM tb_account_supplier");
	$select->execute();
	while ($row = $select->fetch(PDO::FETCH_OBJ)) 
	{
		...
		$subdata[]= '<center>
						<font class="text-black" size="2">
							<a href="accounts_payables_view_supplier.php?id='.$row->id.'">'.$row->name.'</a>
						</font>
					</center>';
		...
		<center>
			<div class="btn-group">
				...
				<a href="accounts_payables_view_supplier.php?id='.$row->id.'" title="View supplier data" class="btn btn-primary btn-sm">
					<span class="fa fa-eye"></span>
				</a>

				<a href="accounts_payables_edit_supplier.php?id='.$row->id.'" title="Edit supplier data" class="btn btn-success btn-sm">
