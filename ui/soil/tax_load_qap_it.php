...
{
  $select1 = $pdo->prepare("SELECT SUM(withholding_total_cwt) as total_cwt_withheld FROM tb_tax_purchase where branch_id = '$branchid' and date between '$jandaysfrom' and '$jandaysto' and cwt_percent != '0' ORDER by date ASC ");
  $select1->execute();
  $row1 = $select1->fetch(PDO::FETCH_OBJ);

  $select2 = $pdo->prepare("SELECT SUM(withholding_total_cwt) as total_cwt_withheld FROM tb_tax_vat_purchase where branch_id = '$branchid' and date between '$jandaysfrom' and '$jandaysto' and cwt_percent != '0' ORDER by date ASC ");
  $select2->execute();
  $row2 = $select2->fetch(PDO::FETCH_OBJ);

  $select3 = $pdo->prepare("SELECT SUM(withholding_total_cwt) as total_cwt_withheld FROM tb_tax_non_vat_purchase where branch_id = '$branchid' and date between '$jandaysfrom' and '$jandaysto' and cwt_percent != '0' ORDER by date ASC ");
  $select3->execute();
  $row3 = $select3->fetch(PDO::FETCH_OBJ);


  $array['tax_purchase'] = $row1->total_cwt_withheld;
  $array['tax_vat_purchase'] = $row2->total_cwt_withheld;
  $array['tax_non_vat_purchase'] = $row3->total_cwt_withheld;
...
<table  border="1" width="100%">
            ...
            <tr align="center">
              ...
              <td><font size="2"><?= $rowjanselect->cwt_percent.'%' ?></font></td>
              <td><font size="2"><?= number_format($rowjanselect->withholding_total_cwt,2) ?></font></td>
            </tr>
