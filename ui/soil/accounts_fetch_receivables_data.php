...
include_once 'function.php';
	...
	$customer_id = $_GET['supplier_id'];	

	function fetch_total_paid($pdo,$id)
	{

		$select = $pdo->prepare("SELECT SUM(amount) as total_paid FROM tb_account_customer_payment where account_customer_data_id = '$id' ");
		$select->execute();
	...
	$select = $pdo->prepare("SELECT * FROM tb_account_customer_data where account_customer_id = '$customer_id' ");
	$select->execute();
