...
function fetch_sales_data($pdo,$id)
{
	...
	$select1 = $pdo->prepare("SELECT * FROM tb_tax_non_vat_purchase WHERE id = '$id' ");
	$select1->execute();
	...
	$select = $pdo->prepare("SELECT * FROM tb_tax_non_vat_purchase");
	$select->execute();
	while ($row = $select->fetch(PDO::FETCH_OBJ)) 
	{
		...
		<center>
			<div class="btn-group">
				...
				<a href="tax_view_non_vat_purchase2.php?id='.$row->id.'" class="btn btn-primary btn-sm">
						<span class="fa fa-list"  data-toggle="tooltip" title="View" ></span>
				</a>

				<a href="tax_edit_non_vat_purchase2.php?id='.$row->id.'" class="btn btn-success btn-sm"><span class="fa fa-edit"  data-toggle="tooltip" title="Edit" ></span></a>

				<button data-id='.$row->id.'  class="btn btn-danger btn-sm" id="btn_delete_non_vat_purchase"><span class="fa fa-trash" style="color:#ffffff" data-toggle="tooltip" title="Delete" ></span></button>
