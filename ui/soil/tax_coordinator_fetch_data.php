...
{
	$select1 = $pdo->prepare("SELECT * FROM tb_coordinator WHERE user_id = '$useridphp' AND business_id = '".$row['id']."' ");
	$select1->execute();
...
foreach($result1 as $row1)
{
	...
	$select1 = $pdo->prepare("SELECT * FROM tb_coordinator WHERE user_id = '$useridphp' AND branch_id = '".$row1['id']."' ");
	$select1->execute();
	$select1count = $select1->rowCount();
