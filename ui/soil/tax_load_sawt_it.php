...
{
  $select1 = $pdo->prepare("SELECT SUM(withholding_total_cwt) as total_cwt_withheld FROM tb_tax_sales where branch_id = '$branchid' and date between '$jandaysfrom' and '$jandaysto' and cwt_percent != 0 ORDER by date ASC ");
  $select1->execute();
  $row1 = $select1->fetch(PDO::FETCH_OBJ);
  ...
  $array['withhold_total'] = $row1->total_cwt_withheld;
