...
function fetch_sales_data($pdo,$id)
{
	...
	$select1 = $pdo->prepare("SELECT * FROM tb_tax_vat_purchase WHERE id = '$id' ");
	$select1->execute();
	...
	$select = $pdo->prepare("SELECT * FROM tb_tax_vat_purchase");
	$select->execute();
	while ($row = $select->fetch(PDO::FETCH_OBJ)) 
	{
		...
		// $subdata[]= '<center><font class="text-success" size="2">'.number_format($row->gross_amount,2).'</font></center>';
		// $subdata[]= '<center><font class="text-danger" size="2">'.number_format($row->vat,2).'</font></center>';
		// $subdata[]= '<center><font class="text-warning" size="2">'.number_format($row->net_amount,2).'</font></center>';
		// $subdata[]= '<center><font class="text-black" size="2">'.number_format($row->withholding_total_cwt,2).'</font></center>';
		// $subdata[]= '<center><font class="text-black" size="2">'.number_format($row->withholding_total_vwt,2).'</font></center>';
		...
		<center>
			<div class="btn-group">
				...
				<a href="tax_view_vat_purchase2.php?id='.$row->id.'" class="btn btn-primary btn-sm">
						<span class="fa fa-list"  data-toggle="tooltip" title="View" ></span>
				</a>

				<a href="tax_edit_vat_purchase2.php?id='.$row->id.'" class="btn btn-success btn-sm"><span class="fa fa-edit"  data-toggle="tooltip" title="Edit" ></span></a>

				<button data-id='.$row->id.'  class="btn btn-danger btn-sm" id="btn_delete_vat_purchase"><span class="fa fa-trash" style="color:#ffffff" data-toggle="tooltip" title="Delete" ></span></button>
